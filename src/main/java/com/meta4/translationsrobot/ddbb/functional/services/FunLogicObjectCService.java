...
import com.meta4.translationsrobot.ddbb.functional.repositories.LogicObjectCRepository;
...
@Service
public class FunLogicObjectCService {
    @Autowired
    private LogicObjectCRepository repository;
    private List<M4RdcLogicObject> findedRows;
    ...
    public void getRow(LinkedListObject linkedListObject) {
        ...
        try {
            M4RdcLogicObject row = findedRows.get(0);
    ...
    public List<M4RdcLogicObject> findByIdObject(String idObject) {
        return repository.findByIdObject(idObject);
    }

    public void solveQuery(M4RdcLogicObject row, String language, ObjectDTO aux) throws Exception {
        switch (language) {
            case "USA":
            case "ENG":
